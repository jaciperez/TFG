...
def guardar_volante():
    ...
    if not paciente:
        ...
        return redirect(url_for('volantes.nuevo_volante'))
...
def editar_volante(id):
    ...
    if request.method == 'POST':
        ...
        if not paciente:
            ...
            return redirect(url_for('volantes.editar_volante', id=id))
        ...
        db.session.commit()
        # registrar edición
        log = LogActividad(usuario_id=current_user.id, accion=f"Editó volante ID {volante.id}")
        db.session.add(log)
        db.session.commit()
...
def eliminar_volante(id):
    ...
    # Log de actividad
    log = LogActividad(usuario_id=current_user.id, accion=f"Eliminó volante ID {id}")
    db.session.add(log)
    db.session.commit()
